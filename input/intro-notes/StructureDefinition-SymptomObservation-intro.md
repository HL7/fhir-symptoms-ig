...
1. Trend - the intensity of the symptom over time.  This is represented by the LOINC code 89253-9 and a value that is the trend.
...
1. Triggers or Exacerbating Factors - these are factors which either trigger the symptom or exacerbate the symptom.  This is represented by the LOINC code 100752-5 and a value that is the factor.  It also provides an extension to indicate a medication that is believed to cause or exacervate the symptom.
1. Alleviating Factors - there are factors that decrease the symptom.  This is represented by the LOINC code 100753-3 and a value that is the factor.  It also provides an extension to indicate a medication that is believed to alleviate the symptom.
